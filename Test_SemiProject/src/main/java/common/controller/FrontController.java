...
@WebServlet(
		description = "사용자가 웹에서 *.bz 을 했을 경우 이 서블릿이 응답을 해주도록 한다.", 
		urlPatterns = { "*.bz" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/git/Test_SemiProject/Test_SemiProject/src/main/webapp/WEB-INF/Command.properties", description = "*.up 에 대한 클래스의 매핑파일") 
		})
	...
	*/
	...
	//	System.out.println("~~~ 확인용 props => " + props);
		 ...
		 */	
			...
			System.out.println(">>> C:/git/Test_SemiProject/Test_SemiProject/src/main/webapp/WEB-INF/Command.properties 파일이 없습니다.<<<");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
